...
function GetCustomerCouponsByMaxPrice(): JSX.Element {
    ...
    const send = (data: GetCoupon) => {
        jwtAxios.get<Coupon[]>(globals.customer.getCouponsByMaxPrice + data.maxPrice)
        .then(res => setCoupons(res.data))
        .catch(err => notify.error("Error getting coupons by max price..."));
    }
    
    return (
        <div className="getCustomerCouponsByMaxPrice" dir="rtl">
            ...
            <div className="CouponsList">
                {coupons.map(item=><SingleCoupon key={item.id} coupon={item} updateCoupon={() => {}} 
                // FIX: Removed couponPurchased={false}
                isOwned={true} // Customer always views owned coupons here
                />)}
            </div>
        </div>
    );
